...
class CIHelper:
    ...
    def db_config(self):
        if not self._db_config:
            self._db_config = DbConfigurator(self.example_config, self.example_section)
    ...
    def buildDataFrame(self, oauth, is_lippincott):
        ...
        info_api = "https://example.libwizard.com/api/v1/public/forms/{}".format(str(form_id))
        ...
        if is_lippincott:
            ...
            print("====> Loading example form information.")
        ...
        while paginate:
            ...
            entry_api = f"https://example.libwizard.com/api/v1/public/submissions/{str(form_id)}"
            # Run the pagination
    ...
    def get_path(self, temp_dir, is_lippincott):
        ...
        file_name = 'lippincott' if is_lippincott else 'example'
        file_name += '_%s.csv' % date_component
        return Path(temp_dir).joinpath(file_name).as_posix()

    def uploadToexample(self, data_frame):
        logging.debug('uploadToexample()')
        ...
        logging.warning('Loading records into example.')
        cursor.execute("ROLLBACK;")
    ...
    def run_job(self):
        ...
        # upload data to example
        if lippincott is not None:
            print('====> Loading Lippincott records.')
            self.uploadToexample(lippincott)
            print('====> Lippincott records saved.')
        # fetch data for example
        print('==> Fetching example records.')
        example = self.buildDataFrame(oauth, False)
        if example is not None:
            print('====> Loading example records.')
            self.uploadToexample(example)
            print('====> example records saved.')
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        "Fetch consultation and instruction data from SpringShare and upload to example via a csv")
    ...
    parser.add_argument("--example-config",
                        help="Configuration for example Connection",
